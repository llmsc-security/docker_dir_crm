#!/usr/bin/env python3
"""Generate Playwright test files for all repositories."""
...
for repo, port in repos.items():
    test_file = os.path.join(script_dir, f"tutorial_{repo}_playwright.py")

    if not os.path.exists(test_file):
        content = template.format(repo=repo, port=port)
        with open(test_file, "w") as f:
            f.write(content)
        os.chmod(test_file, 0o755)
        print(f"  Created: tutorial_{repo}_playwright.py (port {port})")
        generated += 1

print(f"\\nGenerated {generated} new Playwright test scripts")
