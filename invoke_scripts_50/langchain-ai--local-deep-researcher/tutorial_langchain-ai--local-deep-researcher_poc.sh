#!/bin/bash
# Tutorial PoC script for langchain-ai--local-deep-researcher
# Tests the HTTP service endpoint

set -e

# Absolute paths
REPO_DIR="/home/taicen/wangjian/os_dev_google/docker_dirs_yuelin/repo_dirs/langchain-ai--local-deep-researcher"
SCRIPTS_DIR="/home/taicen/wangjian/os_dev_google/docker_dirs_yuelin/invoke_scripts_50/langchain-ai--local-deep-researcher"
BUILD_LOG_DIR="/home/taicen/wangjian/os_dev_google/docker_dirs_yuelin/build_logs"
BUILD_LOG="${BUILD_LOG_DIR}/langchain-ai--local-deep-researcher.build.log"

# Port mapping from port_mapping_50_gap10_1.json
# langchain-ai--local-deep-researcher maps to base port 11030
HOST_PORT=11030
...
log_message "Starting tutorial PoC script for langchain-ai--local-deep-researcher"
log_message "Target service: http://127.0.0.1:${HOST_PORT}"

echo ""
echo "=============================================="
echo "langchain-ai--local-deep-researcher PoC - Testing Service Endpoints"
echo "=============================================="
echo ""
...
# Test 3: Check langgraph endpoints
log_message "Test 3: LangGraph API endpoint"
echo ""
echo "Test 3: Checking LangGraph API..."
LANGGRAPH_RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" "http://127.0.0.1:${HOST_PORT}/health" 2>/dev/null || echo "N/A")
echo "  LangGraph endpoint HTTP status: ${LANGGRAPH_RESPONSE}"
log_message "  LangGraph check: COMPLETE"
...
echo "Test 3 - LangGraph API: ${LANGGRAPH_RESPONSE}"
echo ""
