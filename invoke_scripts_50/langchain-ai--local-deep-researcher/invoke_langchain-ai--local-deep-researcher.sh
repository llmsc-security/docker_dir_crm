#!/bin/bash
# Invoke script for langchain-ai--local-deep-researcher
# Tests the HTTP service endpoint

set -e

REPO_NAME="langchain-ai--local-deep-researcher"
IMAGE_NAME="langchain-ai--local-deep-researcher_image"
CONTAINER_NAME="langchain-ai--local-deep-researcher_container"
PORT=11030
HOST="localhost"
CONTAINER_PORT=2024

echo "============================================"
echo "Testing langchain-ai--local-deep-researcher on port $PORT"
echo "============================================"
...
if docker ps --format '{{.Names}}' | grep -q "${CONTAINER_NAME}"; then
    ...
    docker run -d \
        ...
        -e LANGCHAIN_HOST=0.0.0.0 \
        -e LANGCHAIN_PORT=${CONTAINER_PORT} \
        --rm \
        "${IMAGE_NAME}:latest" || {
