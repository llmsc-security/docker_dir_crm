#!/bin/bash
# Invoke script for gptme--gptme
# Tests the HTTP service endpoint

set -e

REPO_NAME="gptme--gptme"
IMAGE_NAME="gptme--gptme_image"
CONTAINER_NAME="gptme--gptme_container"
PORT=11130
HOST="localhost"
CONTAINER_PORT=11130

echo "============================================"
echo "Testing gptme--gptme on port $PORT"
echo "============================================"
...
if docker ps --format '{{.Names}}' | grep -q "${CONTAINER_NAME}"; then
    ...
    docker run -d \
        ...
        -e GPTME_SERVER_PORT=${CONTAINER_PORT} \
        --rm \
        "${IMAGE_NAME}:latest" || {
