...
test_main_service() {
    ...
    # Check for dataflow in content
    if echo "$content" | grep -qi "dataflow\|status"; then
        log "INFO" "Web interface responding with expected content"
        return 0
    else
        log "WARNING" "Main service test - content may not match expected pattern"
        return 0
    fi
}
