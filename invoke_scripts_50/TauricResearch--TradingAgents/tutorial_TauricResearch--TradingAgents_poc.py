...
# -*- coding: utf-8 -*-
"""
Tutorial PoC script for TauricResearch--TradingAgents
...
def test_main_service():
    """Test the main API endpoints (FastAPI/Gradio)."""
    try:
        # Test a simple endpoint - try the stock endpoint
        url = f"http://{HOST}:{PORT}/api/stock"
        req = urllib.request.Request(url)
        with urllib.request.urlopen(req, timeout=10) as response:
            content = response.read().decode()
            log(f"API stock endpoint test - Status: {response.status}")
            return True
    except urllib.error.HTTPError as e:
        if e.code == 404:
            log("Main API endpoint not found (optional)", "WARNING")
            return True
